...
class AsyncWikipediaSearchTool(AsyncTool):
    """
    ...
    async def forward(self, query: str) -> str:
        try:
            # Fetch the page object (network I/O) in a thread to avoid blocking
            page = await asyncio.to_thread(self.wiki.page, query)

            # Check existence (synchronous check)
            if not page.exists():
                return f"No Wikipedia page found for '{query}'. Try a different query."

            # Retrieve metadata and content synchronously
            title = page.title
            url = page.fullurl
...
class AsyncSpeechToTextTool(AsyncPipelineTool):
    ...
    def encode(self, audio):
        ...
        audio_obj = AgentAudio(audio).to_raw()
        return self.pre_processor(audio_obj, return_tensors="pt")

    async def forward(self, inputs) -> Any:
